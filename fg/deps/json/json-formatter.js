...
angular.module('jsonFormatter', ['RecursionHelper'])
	...
	.directive('jsonFormatter', ['RecursionHelper', 'JSONFormatterConfig', function jsonFormatterDirective(RecursionHelper, JSONFormatterConfig) {
		...
		function link(scope, element) {
			...
			//szz
			scope.isHeaderValue = function () {
				return scope.type === 'string' && scope.hasKey && scope.key == "value";
			};

			//szz
			scope.isHeader = function () {
				return scope.getConstructorName() == "Header";
			};
			...
			if (scope.type === 'string') {
				...
				//szz
				scope.isHeaderName = scope.hasKey && scope.key == "name";

				//szz
				if (scope.isHeaderValue()) {
					
					element.keypress(ev => { return ev.which != 13 && scope.request.adding; });

					element.on('input', event => {
						
						let header_name = element.closest('.header-bucket').find('.headerName').text().trim();
						...
						scope.toID = setTimeout(() => {
							...
							let value = element.text().split(":").slice(1).join(":").trim();
							
							scope.request.events.emit('header-change', value, JSON.parse(header_name));
							
							element.closest('.header-row').trigger('value-change', [value]);
			...
			} else if (scope.isHeader()) {
				
				element.on('value-change', (event, new_value) => {
					
					scope.json.value = new_value;
			...
			scope.$watch("open",
						 (newValue, oldValue) =>  {
							 ...
							 if (newValue !== oldValue) {
								 ...
								 //szz: isArray == requestHeaders;
								 if (scope.isArray()) {
									 if ((newValue > oldValue && !scope.isOpen) || (newValue < oldValue && scope.isOpen))
										 scope.toggleOpen();
								 }
							 }
						 });
...
angular.module("jsonFormatter").run(["$templateCache", function($templateCache) {$templateCache.put("json-formatter.html","<div ng-init=\"isOpen = open && open > 0\" class=\"json-formatter-row {{isHeader() ? \'header-row\' : \'\'}}\"><a ng-click=\"toggleOpen()\"><span class=\"toggler {{isOpen ? \'open\' : \'\'}}\" ng-if=\"isObject()\"></span> <span class=\"key\" ng-if=\"hasKey\"><span class=\"key-text\">{{key}}</span><span class=\"colon\">:</span></span> <span class=\"value\"><span ng-if=\"isObject()\"><span class=\"constructor-name\">{{getConstructorName(json)}}</span> <span ng-if=\"isArray()\"><span class=\"bracket\">[</span><span class=\"number\">{{json.length}}</span><span class=\"bracket\">]</span></span></span><span ng-if=\"!isObject() && !isHeaderValue()\" ng-click=\"openLink(isUrl)\" class=\"{{type}}\" ng-class=\"{date: isDate, url: isUrl, headerName: isHeaderName}\">{{parseValue(json)}}</span><bdi contenteditable=true ng-if=\"!isObject() && isHeaderValue()\" class=\"{{type}}\">{{json}}</bdi></span><span ng-if=\"showThumbnail()\" class=\"thumbnail-text\">{{getThumbnail()}}</span></a><div class=\"children {{isHeader() ? \'header-bucket\' : \'\'}}\" ng-if=\"getKeys().length && isOpen\"><json-formatter ng-repeat=\"key in getKeys() track by $index\" json=\"json[key]\" key=\"key\" open=\"childrenOpen()\" urlclick=\"urlclick\" request=\"request\"></json-formatter></div><div class=\"children empty object\" ng-if=\"isEmptyObject()\"></div><div class=\"children empty array\" ng-if=\"getKeys() && !getKeys().length && isOpen && isArray()\"></div></div>");}]);
